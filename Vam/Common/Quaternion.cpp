...
Quaternion Quaternion::FromToRotation(VECTOR fromDir, VECTOR toDir)
{
    ...
    return Quaternion::AngleAxis(Deg2RadD(angle), axis);
...
bool Quaternion::EqualsVZero(const VECTOR& v1)
{
    const VECTOR& v2 = { 0.0f, 0.0f, 0.0f };
...
double Quaternion::SqrMagnitude(const VECTOR& v)
{
    return v.x * v.x + v.y * v.y + v.z * v.z;
}

double Quaternion::Deg2RadD(double deg)
{
...
double Quaternion::AngleDeg(const VECTOR& from, const VECTOR& to)
{
...
VECTOR Quaternion::VNormalize(const VECTOR& v)
{
    if (EqualsVZero(v))
    {
...
double Quaternion::RadIn2PI(double rad)
{
